...
"""
soft , b = stats.boxcox(train['房屋面积'])
soft = list(soft)
train.drop('房屋面积',axis=1, inplace=True)
train['房屋面积'] = soft
soft , b = stats.boxcox(test['房屋面积'])
soft = list(soft)
test.drop('房屋面积',axis=1, inplace=True)
test['房屋面积'] = soft
del soft, b
...
num = len(test_ws)
for i in range(num):
    temp = test_ws[i] + test_t[i]
    total.append(temp)
test['卧室和厅'] = total
del i, num, temp, test_t, test_ws, train_t, train_ws, total
"""
# 这个特征保留
# 卧室占整个房间的比例
bed_sub_all = 0.3
...
"""
# 不上分，卫生间面积相关的融合会掉分
train_wss = list(copy.deepcopy(train['卫的均面积']))
train_ws = list(copy.deepcopy(train['卫的数量']))
train_wsz = list(copy.deepcopy(train['卧室总面积']))
ws_s = []
num = len(train_wss)
for i in range(num):
    temp = train_wss[i] * train_ws[i] + train_wsz[i]
    ws_s.append(temp)
train['卫生间和卧室总面积'] = ws_s
test_wss = list(copy.deepcopy(test['卫的均面积']))
test_ws = list(copy.deepcopy(test['卫的数量']))
test_wsz = list(copy.deepcopy(test['卧室总面积']))
ws_s = []
num = len(test_wss)
for i in range(num):
    temp = test_wss[i] * test_ws[i] + test_wsz[i]
    ws_s.append(temp)
test['卫生间和卧室总面积'] = ws_s
del train_wss, train_ws, ws_s, num, i, temp, test_wss, test_ws, train_wsz, test_wsz
"""
# 保留，可能有效
"""
cross-validation train scores:
[2.29826756 2.25111423 2.35636127 2.29860962 2.31068692]
cross-validation test scores:
[2.66811465 2.60585486 2.6789672  2.77267486 2.58256344]
"""
...
num = len(train_sq)
for i in range(num):
    temp = (train_sq[i] * 10000 * 0.4) / (train_ws[i] + 1)
    ws_sq.append(temp)
train['客厅均面积'] = ws_sq
test_sq = list(copy.deepcopy(test['房屋面积']))
...
num = len(test_sq)
for i in range(num):
    temp = (test_sq[i] * 10000 * 0.4) / (test_ws[i] + 1)
    ws_sq.append(temp)
test['客厅均面积'] = ws_sq
del train_sq, train_ws, ws_sq, i, temp, num, test_sq, test_ws
"""
...
# 都掉分
"""
train['有无卫生间'] = (train['卫的数量'] != 0) * 1
test['有无卫生间'] = (test['卫的数量'] != 0) * 1
"""
"""
train['有无卧室'] = (train['卧室数量'] != 0) * 1
test['有无卧室'] = (test['卧室数量'] != 0) * 1
"""
"""
train['有无客厅'] = (train['厅的数量'] != 0) * 1
test['有无客厅'] = (test['厅的数量'] != 0) * 1
"""
"""
train['有无地铁'] = (train['地铁线路'] != 0) * 1
test['有无地铁'] = (test['地铁线路'] != 0) * 1
"""
"""
train['有无装修'] = (train['装修情况'] != 0) * 1
test['有无装修'] = (test['装修情况'] != 0) * 1
"""
...
xgb_clf=xgb.XGBRegressor(max_depth=8,
                         ...
                         silent=1, 
                         n_jobs=-1)
xgb_clf.fit(train,train_label)
predict = xgb_clf.predict(test)
test_lgb = pd.DataFrame({'id': test_id, 'price': predict})
test_lgb.to_csv('../result/xgb_predict.csv', index=False)
